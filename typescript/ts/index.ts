...
let age:number; // age will only hold a number 
let passed : boolean; // passed will only hold true or false
let stdName : string; // stdName will only hold a string
let students : object[]; 
...
let mixed : (string|number|boolean)[] = []; // this array can take string, numbers and booleans
...
const multipleAddition = (...a:number[]) => {
    if(a){
        let total = 0;
...
let greetAgain = (user:{name:string, uid:string | number}):void => {
    console.log(`The user id of ${user.name} is ${user.uid}`);
}
// as you can see the uid type is string | number, is taking too much space we can redce this see below :- 
type strOrNum = string | number;
greetAgain = (user:{name:string, uid:strOrNum}):void => {
    console.log(`The user id of ${user.name} is ${user.uid}`);
}
...
greetAgain = (user:objWithName):void => {
    console.log(`The user id of ${user.name} is ${user.uid}`);
}
...
let greetOnceMore : (a:string, b:string) => void; // this line of code says greetOnceMore will only hold those function which is taking two parameter and both are strings also the reture type of this function will be void.
greetOnceMore = (Name:string, greeting:string) => {
